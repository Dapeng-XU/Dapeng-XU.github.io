...
\begin{frame}{Demo: Gabow's biconnected components algorithm}
	...
	\begin{itemize}
		...
		\only<2>{DFS(1): w=2}
		\only<3>{DFS(2): w=3}
		\only<4>{DFS(3): w=1}\only<5>{DFS(3): w=4}\only<6>{DFS(4): w=2}\only<7>{DFS(4): w=5}\only<8>{DFS(5): w=3}\only<9>{DFS(5): w=6}\only<10>{DFS(6): w=7}
		\only<11>{DFS(7): w=5}
		\only<12>{DFS(7): End}
		\only<13>{DFS(6): End}
...
\begin{frame}{Correctness}
	...
	\begin{theorem}[Correctness and complexity]
		...
		The time and space are both $O(m+n)$.
	\end{theorem}
\end{frame}
